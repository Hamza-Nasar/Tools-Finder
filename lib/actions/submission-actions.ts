...
export async function submitToolAction(_: ActionState, formData: FormData): Promise<ActionState> {
  try {
    ...
    if (isDatabaseUnavailableError(error)) {
      return {
        status: "error",
        message: "Submission service is temporarily unavailable. Please try again in a few minutes."
      };
    }

    return toActionState(error, "Unable to submit right now. Please try again.");
  }
}
