            ...
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                <Link href={`/categories/${tool.categorySlug}`} className="inline-flex">
                  <Badge
                    variant="muted"
                    className="interactive-chip border border-border/70 bg-white/[0.82] hover:bg-primary hover:text-primary-foreground"
                  >
                    {tool.category}
                  </Badge>
                </Link>
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <Button asChild variant="outline" size="sm" className="group/button">
              <Link href={`/tools/${tool.slug}`}>
                View details
                <ArrowUpRight data-icon="inline-end" className="transition-transform duration-300 group-hover/button:translate-x-0.5 group-hover/button:-translate-y-0.5" />
              </Link>
            </Button>
          </div>
        </CardFooter>
